...
| D5 | Stack | Laravel 13, Pest for tests, Postmark (email), Twilio (SMS), Stripe (payments). **UI is split (owner directive 2026-08-19): the public site is Blade + Livewire + Flowbite on Tailwind 4; the `/admin` backend is Filament v5.** *(History: "Fluent UI" in the earliest notes meant Filament; on 2026-08-16 the owner directed that all UI — public site included — be Filament, and the site was built that way. The 2026-08-19 directive reverses that for the public side only. The rep portal's classification is still open — see [02-architecture.md](02-architecture.md).)* |
