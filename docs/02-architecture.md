...
| Admin UI | **Moving off Filament, one resource at a time** (doc 13). The fair's own screens are Livewire on `uclemmer/laravel-ui` at `/staff`; laravel-core's Filament panel keeps `/admin` for users, roles, the email log, content and settings until core goes headless | Both are live and both ask the same `admin.access` permission, so whoever can reach one can reach the other |
...
| Surfaces | **`/admin`** = laravel-core's prebuilt Filament panel (`core.admin.enabled = true`, branded via `core.admin.brand`), with the fair's not-yet-migrated resources still attached through `core.admin.plugins`. **`/staff`** = the fair's own screens, Livewire (doc 13). **`/portal`** = the representative portal, Livewire since 2026-08-21 (doc 12); its Filament panel is gone. | `App\Filament\FairPlugin` still registers Events, Registrations, Organizations, Grants, FAQ and Messages plus the dashboard widgets. Sponsors has left it for `/staff`; the plugin and its config entry go when the last one does |
