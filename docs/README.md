...
| [18-participant-export.md](18-participant-export.md) | The owner's roster export finally arrived (answering standing question A3) — as form submissions, not the CSV the importer was built for. What it contains, the two seeders that read it, and every judgement in between: why 381 submissions are 354 registrations, which spellings are one organization and which university must not be folded into its own colleges, why a representative's address ends up in `admissions_email`, and the three registration fields that are convention rather than fact |
| [19-admissions-offices.md](19-admissions-offices.md) | Filling in what the export could not: the admissions office behind 156 of the 157 organizations — the office and not the university, why nothing is derived from an email domain and no individual's address was recorded, the narrow rule that swaps a representative's address for the office inbox without touching anything a human typed, and `fair:fetch-organization-logos`, which discovers each logo from the institution's own metadata rather than from a hand-collected list that would rot |
