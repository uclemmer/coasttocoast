...
| [17-school-to-organization-rename.md](17-school-to-organization-rename.md) | Renaming schools to organizations: why no migration or model rename was needed (the schema had said `organizations` since card 1.2), the two senses of "school" that stay — the four sponsoring preparatory schools and the high-school visitors — the 148 articles a mechanical pass got wrong, and the published `vendor/core` view that only the guard test found |
| [18-participant-export.md](18-participant-export.md) | The owner's roster export finally arrived (answering standing question A3) — as form submissions, not the CSV the importer was built for. What it contains, the two seeders that read it, and every judgement in between: why 381 submissions are 353 registrations, which spellings are one organization and which university must not be folded into its own colleges, why a representative's address ends up in `admissions_email`, and the three registration fields that are convention rather than fact |
| [19-admissions-offices.md](19-admissions-offices.md) | Filling in what the export could not: the admissions office behind every one of the 156 organizations — the office and not the university, why nothing is derived from an email domain and no individual's address was recorded, the narrow rule that swaps a representative's address for the office inbox without touching anything a human typed, and `fair:fetch-organization-logos`, which discovers each logo from the institution's own metadata rather than from a hand-collected list that would rot |
| [20-postmaster-06-upgrade.md](20-postmaster-06-upgrade.md) | Postmaster `0.2` → `0.6` in one step: why the `0.4.0` breaking change and the `0.6.0` bounce-policy reversal both missed this app, the migration a `composer update` does not bring, and the config refresh that reset the master switch and took the send-time suppression guard down with it — including why the diff that was supposed to catch that could not, and the admin URI collision guard added because one provider's surface tripled |
| [21-core-06-upgrade.md](21-core-06-upgrade.md) | Core `0.5.1` → `0.6.0`: a version-only bump, and the finding it surfaced — this app's `robots.txt` is the stock one that permits everything, with the admin at `/admin` |
