...
use App\Livewire\Staff\Events\Show as ViewEvent;
...
describe('announcing that registration is open', function () {
    it('mails everyone waiting and stamps them', function () {
        ...
        livewire(ViewEvent::class, ['event' => $this->fair])->call('announce');
    ...
    it('does nothing at all on a second run', function () {
        ...
        expect(livewire(ViewEvent::class, ['event' => $this->fair->refresh()])->instance()->canAnnounce())
            ->toBeFalse();
    ...
    it('is not offered when nobody is waiting', function () {
        expect(livewire(ViewEvent::class, ['event' => $this->fair])->instance()->canAnnounce())->toBeFalse();
    });

    it('is not offered on an unpublished fair', function () {
        // Announcing a draft would send a hundred people to a 404.
        $draft = Fair::factory()->create();
        EventInterest::factory()->for($draft)->create();

        expect(livewire(ViewEvent::class, ['event' => $draft])->instance()->canAnnounce())->toBeFalse();
    });
});
